...
>(({ className, children, ...props }, ref) => (
  ...
  >
    {children}
    {/* Chevron можно скрыть в CSS: .selectTrigger svg { display: none } */}
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      // 👇 ЭТО ГЛАВНОЕ: реальный отступ между триггером и дропдауном (8px)
      sideOffset={4}
      className={cn(
        "relative z-50 min-w-[8rem] overflow-hidden rounded-md border bg-popover text-popover-foreground shadow-md",
        "data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 origin-[--radix-select-content-transform-origin]",
        // убрали все translate-y, чтобы не мешали воспринимать отступ
        position === "popper" &&
      ...
      {...props}
    >
      {/* Внешний контейнер — задаёт высоту */}
      <div className="h-[244px]">
        {/* Внутренний — скроллится и немного уже (mr-2), чтобы скролл был на 8px левее края */}
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      // серый фон на focus / highlight оставляем
